	...
	public void updateMiniBatch(ArrayList<CharImgType> miniBatch, Double eta) {
    	...
    	for (int i = 0; i < miniBatch.size(); i++) {
			...
			for (int j = 0; j < biases.size(); j++) {
				for (int k = 0; k < biases.get(j).length; k++) {
			...
			nabla_w.forEach((nw)->{
				for (int i = 0; i < w.length; i++) {
					for (int j = 0; j < w[i].length; j++) {
						w[i][j] = w[i][j] - ((eta/miniBatch.size()) * nw[i][j]); 
					}
				}
			});
		});
		...
		biases.forEach((b)-> {
			nabla_b.forEach((nb)->{
				for (int i = 0; i < b.length; i++) {
					b[i] = b[i] - ((eta/miniBatch.size()) * nb[i]);
				}
			});
		});
   	}
	
   	public void backpropagation(CharImgType data,
		ArrayList<Double[]> biases, ArrayList<Double[][]> weights) {
		//FEED FORWARD
